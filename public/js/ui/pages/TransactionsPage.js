...
class TransactionsPage {
  ...
  constructor(element) {
    if (!element) {
      throw new Error('Элемент в TransactionsPage не задан');
    } else {
    ...
    const clickHandler = (evt) => {
      ...
      if (evt.target.closest('.transaction__remove')) {
        this.removeTransaction(
          evt.target.closest('.transaction__remove').dataset.id
        );
      }
    };
  ...
  formatDate(date) {
    ...
    const day = inputDate.toLocaleString('ru-Ru', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });
    const time = inputDate.toLocaleString('ru-Ru', {
      hour: 'numeric',
      minute: 'numeric',
    });
    return `${day} в ${time}`;
